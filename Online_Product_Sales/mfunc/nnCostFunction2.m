...
A_3= [ones(size(Z_3,1),1) sigmoid(Z_3)];
Z_4= A_3*Theta3';
A_4= sigmoid(Z_4);    % = H_theta (5000 x 10)

for k = 1:num_labels
    J = J + sum(-(y==k).*log(A_4(:,k)) - (1-(y==k)).*log(1-A_4(:,k)));
end
...
delta_3 = delta_4*Theta3 .* A_3.*(ones(size(A_3,1),size(A_3,2)) - A_3);% (5000 x 26)
delta_3 = delta_3(:,2:end);% (5000 x 25)


delta_2 = delta_3*Theta2 .* A_2.*(ones(size(A_2,1),size(A_2,2)) - A_2);% (5000 x 26)
